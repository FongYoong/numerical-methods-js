...
const DialogContent =  withStyles((theme) => ({
    ...
    const updateGraph = useCallback(() => {
        if (calculator.current){
            ...
            calculator.current.setExpression({ id: 'derivative', color: Desmos.Colors.GREEN, lineStyle: Desmos.Styles.DOTTED, latex:
                `(y-${formatLatex(currentResult.funcResult)})/(x-${formatLatex(currentResult.previousX)})=${formatLatex((currentResult.funcResult2 - currentResult.funcResult1) / (params.perturbation * currentResult.previousX))}` });
    ...
    }, [calculator, params, currentResult]);
    updateGraph();
