...
function EigenShiftedPower({methodName, markdown}) {
    ...
    // Shift constant
    const [shiftConstant, setShiftConstant] = useState(6.0);
    let shiftError = false;
    let shiftErrorText = "";
    if (isNaN(shiftConstant)) {
        shiftError = true;
        shiftErrorText = "Shift constant must be a number!";
    }
    ...
    let hasError = shiftError || iterError;

    // Solve
    let solve = false;
    const originalMatrix = gridTo2DArray(gridState.rows);
    const shiftedMatrix = subtract(originalMatrix, multiply(shiftConstant, identity([originalMatrix.length])));
    let inverseMatrix, inverseError = false, inverseErrorText = "";
    try {
        inverseMatrix = inv(shiftedMatrix);
    }
    catch {
        inverseError = true;
        inverseErrorText = "Matrix A-aI does not have an inverse!";
        solve = true;
    }
...
function Steps({smallScreen, params}) {
    ...
    if (currentIteration > params.iterations) {
        ...
        \displaystyle
        \begin{array}{l}
        \\ (A - aI) = ${matrixToLatex(params.shiftedMatrix)}
        \\
        \\ (A - aI)^{-1} = ${matrixToLatex(params.inverseMatrix)}
        \\
        \\ \hline
        \begin{array}{lcl}
        ...
        latexContent += String.raw`
        \\ \hline
        \\ \text{Dominant eigenvalue of } (A - aI)^{-1} \text{,}
        \\ \begin{array}{lcl}
        ...
        \\ \end{array}
        \\
        \\ \text{Eigenvalue closest to a,}
        \\ \begin{array}{lcl}
        \\ \lambda_{${currentIteration}} &=& \frac{1}{\lambda_{${currentIteration}}^{-1}} + a
        \\
...
export default EigenShiftedPower;